...
public class ServiceLocator implements ProxyableService, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener {
    ...
    public static final String RECEIVER_ACTION_GEOHASH = "org.owntracks.android.RECEIVER_ACTION_GEOHASH";
    public static final String RECEIVER_ACTION_ADDRESS = "org.owntracks.android.RECEIVER_ACTION_ADDRESS";
    public static final String RECEIVER_ACTION_MEETINGPOINT = "org.owntracks.android.RECEIVER_ACTION_MEETINGPOINT";
    ...
    private void onFenceTransition(Intent intent) {
        ...
        System.out.println("<ServiceLocator> on Fence Transition ");
        Timber.v("");
        if(event != null){
            if(event.hasError()) {
	...
	public void onStartCommand(Intent intent) {
        if(intent == null)
            return;
        System.out.println("<onStartcommand> ServiceLocator");
        ...
        }else if(ServiceLocator.RECEIVER_ACTION_GEOHASH.equals(intent.getAction())){
            reportGeoHash(intent.getExtras().getString("<CMIYC>GeoHashInformation"),intent.getExtras().getString("<CMIYC>messageId"));
        }else if(ServiceLocator.RECEIVER_ACTION_ADDRESS.equals(intent.getAction())){
            reportAddress((HashMap<String[], String>) intent.getSerializableExtra("<CMIYC>nameMobileNumber_encryptedMessage"));
        }else if(ServiceLocator.RECEIVER_ACTION_MEETINGPOINT.equals(intent.getAction())){
            reportMeetingPoint((HashMap<String[], String>) intent.getSerializableExtra("<CMIYC>nameMobileNumber_encryptedMessageMeetingPoint"));
        }
    ...
    private void reportGeoHash(String geoHash, String messageId){
        reportGeoHashLocation(MessageLocation.REPORT_TYPE_USER,geoHash,messageId);
    }

    private void reportAddress(HashMap<String[],String> map){
        reportAddressLocation(MessageLocation.REPORT_TYPE_USER,map);
    }

    private void reportMeetingPoint(HashMap<String[],String> map){
        reportMeetingPointLocation(MessageLocation.REPORT_TYPE_USER,map);
    }
	...
	private void reportGeoHashLocation(String trigger,String geoHash,String messageId){
        ...
        if(ignoreLowAccuracy(l))
            return;
        System.out.println("< REPORTING GEOHASH > "+geoHash);
        Timber.v("trigger:%s", trigger);
        MessageLocation message = new MessageLocation();
        message.setMessageDestinationId(messageId);
        message.setLat(l.getLatitude());
        message.setLon(l.getLongitude());
        message.setMessageType("geohash");
        message.setGeoHashJSON(geoHash);
    ...
    private void reportAddressLocation(String trigger, HashMap<String[],String> map){

        for (String[] s : map.keySet())
            System.out.println("< REPORTING ADDRESS LOCATION > "+s[0]+" "+s[1]+" | "+map.get(s));

        for(String[] name_id : map.keySet()) {
            Timber.v("trigger:%s", trigger);
            MessageLocation message = new MessageLocation();
            message.setMessageDestinationId(name_id[1]);
            message.setMessageType("address");
            message.setAddress(map.get(name_id));
            message.setT(trigger);
            message.setTst(TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis()));
            message.setTid(Preferences.getTrackerId(true));
            message.setCp(Preferences.getCp());
    ...
    private void reportMeetingPointLocation(String trigger, HashMap<String[],String> map){
        for (String[] s : map.keySet())
            System.out.println("< REPORTING MEETINGPOINT LOCATION > "+s[0]+" "+s[1]+" | "+map.get(s));

        for(String[] name_id : map.keySet()) {
            Timber.v("trigger:%s", trigger);
            MessageLocation message = new MessageLocation();
            message.setMessageDestinationId(name_id[1]);
            message.setMessageType("meetingPoint");
            message.setMeetingPoint(map.get(name_id));
            message.setT(trigger);
            message.setTst(TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis()));
            message.setTid(Preferences.getTrackerId(true));
            message.setCp(Preferences.getCp());
